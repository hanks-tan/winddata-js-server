...
function convertGribToJson(stamp, targetMoment){
	...
	var cmdStr = 'converter/bin/grib2json --data --output json-data/'+stamp+'.json --names --compact grib-data/'+stamp+'.f000';
	if(path.sep === '\\') {
		cmdStr = 'converter\\bin\\grib2json --data --output json-data\\'+stamp+'.json --names --compact grib-data\\'+stamp+'.f000';
	}

	child = exec(cmdStr,{maxBuffer: 500*1024},
